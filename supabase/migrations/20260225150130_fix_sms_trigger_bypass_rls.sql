/*
  # Fix SMS Trigger to Bypass RLS
  
  1. Changes
    - Disable RLS enforcement for the trigger function using SET SESSION
    - This allows system triggers to write logs even when no user is authenticated
  
  2. Security
    - The function is SECURITY DEFINER so only trusted trigger code can execute it
    - The function still validates SMS settings before sending
*/
...
BEGIN
  IF NEW.sent_to_doctor_at IS NOT NULL AND (OLD.sent_to_doctor_at IS NULL OR OLD.sent_to_doctor_at != NEW.sent_to_doctor_at) THEN
    BEGIN
      ...
      -- Check if we already sent SMS for this visit (bypass RLS for check)
      SELECT EXISTS(
      ...
      -- Insert log entry bypassing RLS by using SECURITY DEFINER privileges
      -- The function owner has necessary privileges
...
-- Grant the service role permission to bypass RLS on sms_log
ALTER TABLE sms_log FORCE ROW LEVEL SECURITY;

-- Add a policy that allows the function owner (postgres) to insert
CREATE POLICY "Allow function owner to insert logs"
