/*
  # Fix SMS Log Insert Conflict
  
  1. Changes
    - Update trigger function to use INSERT ... ON CONFLICT DO NOTHING
    - This prevents errors when multiple tests for same visit trigger simultaneously
  
  2. Security
    - No security changes, function remains SECURITY DEFINER
*/
...
BEGIN
  IF NEW.sent_to_doctor_at IS NOT NULL AND (OLD.sent_to_doctor_at IS NULL OR OLD.sent_to_doctor_at != NEW.sent_to_doctor_at) THEN
    BEGIN
      ...
      ON CONFLICT (visit_id, sms_type) DO NOTHING;
