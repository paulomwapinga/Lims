/*
  # Fix SMS Triggers to Send Correct Parameters

  1. Changes
    - Update triggers to send api_key, secret_key, and source_addr instead of apiUrl/apiKey
    - Add sms_secret_key and sms_source_addr to settings retrieval
    - Ensure triggers send all required parameters to the send-sms edge function

  2. Notes
    - This fixes the mismatch between what triggers send and what the edge function expects
*/

-- Update function to send SMS when all test results are completed
...
-- Update function to send welcome SMS when a new patient is created
